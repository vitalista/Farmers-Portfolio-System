...
$query = "
    SELECT
        ...
        f.gov_id_type,
        f.gov_id_number,
        f.selected_enrollment,

        p.fps_code AS parcel_code,
        p.parcel_no,
        p.owner_first_name,
        p.owner_last_name,
        p.ownership_type,
    ...
    FROM 
        crops AS c
    JOIN 
        parcels AS p ON p.id = c.parcel_id
    JOIN 
        farmers AS f ON f.id = c.farmer_id
    WHERE 
...
if (!empty($_GET['hvc'])) {
    $whereConditions[] = "c.hvc = ?";
    $params[] = validate($_GET['hvc']);
    $types .= "i"; 
}

if (!empty($_GET['cropname'])) {
    $whereConditions[] = "c.crop_name = ?";
    $params[] = validate($_GET['cropname']);
    $types .= "s";
}
...
if (!empty($_GET['classification'])) {
    $classificationComp = validate($_GET['classificationComp']) ?? 'exact';
    $classification = validate($_GET['classification']);
    
    switch ($classificationComp) {
        case 'lessThan':
            $whereConditions[] = "c.classification < ?";
            break;
        case 'greaterThan':
            $whereConditions[] = "c.classification > ?";
            break;
...
if (!empty($_GET['cropArea'])) {
    $cropAreaComparison = validate($_GET['cropAreaComparison']) ?? 'exact';
    $cropArea = validate($_GET['cropArea']);
    
    switch ($cropAreaComparison) {
        case 'lessThan':
            $whereConditions[] = "c.crop_area < ?";
            break;
        case 'greaterThan':
            $whereConditions[] = "c.crop_area > ?";
            break;
...
if(!empty('created') && isset($_GET['created'])) {
    ...
    switch ($created) {
        case 'today':
            $whereConditions[] = "c.created_at >= CURDATE() 
        ...
        <div class="modal-content">
            ...
            <form id="filterForm" class="row" method="get">
                ...
                <div class="collapse row" id="moreFilters">
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="cropname" class="form-label">Crop Name <strong>(Contains)</strong></label>
                        <input type="text" id="cropname" name="cropname" class="form-control" placeholder="Enter crop name">
                    </div>

                    <div class="col-md-3 mb-3">
                        <label for="cropAreaComparison" class="form-label">Crop Area</label>
                        <div class="input-group">
                            <select id="cropAreaComparison" name="cropAreaComparison" class="form-select">
                                <option value="exact">Is Exact</option>
                                <option value="lessThan">Less Than</option>
                                <option value="greaterThan">Greater Than</option>
                            </select>
                            <input type="number" id="cropArea" name="cropArea" class="form-control" placeholder="Enter">
                        </div>
                    </div>

                    <div class="col-md-3 mb-3">
                        <label for="classificationComp" class="form-label">Classification</label>
                        <div class="input-group">
                            <select id="classificationComp" name="classificationComp" class="form-select">
                                <option value="exact">Is Exact</option>
                                <option value="lessThan">Less Than</option>
                                <option value="greaterThan">Greater Than</option>
                            </select>
                            <input type="number" id="classification" name="classification" min="0" class="form-control" placeholder="Enter">
                        </div>
                    </div>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" name="hvc" style="width: 20px; height: 20px;" type="radio" id="hvcYes" value="1">
                            <label class="form-check-label" for="hvcYes">Yes</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" name="hvc" style="width: 20px; height: 20px;" type="radio" id="hvcNo" value="0">
                            <label class="form-check-label" for="hvcNo">No</label>
                        </div>
                    </div>
