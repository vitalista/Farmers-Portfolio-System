...
$query = "
    ...
    FROM 
        livestocks AS l
    JOIN 
        parcels AS p ON p.id = l.parcel_id
    JOIN 
        farmers AS f ON f.id = l.farmer_id
    WHERE 
...
if (!empty($_GET['livestock'])) {
    $whereConditions[] = "l.animal_name = ?";
    $params[] = validate($_GET['livestock']);
    $types .= "s";
}
...
if (!empty($_GET['numHeads'])) {
    $numHeadsComp = validate($_GET['numHeadsComp']) ?? 'exact';
    $numHeads = validate($_GET['numHeads']);
    
    switch ($numHeadsComp) {
        case 'lessThan':
            $whereConditions[] = "l.no_of_heads < ?";
            break;
        case 'greaterThan':
            $whereConditions[] = "l.no_of_heads > ?";
            break;
...
if(!empty('created') && isset($_GET['created'])) {
    ...
    switch ($created) {
        case 'today':
            $whereConditions[] = "l.created_at >= CURDATE() 
                                  AND l.created_at < CURDATE() + INTERVAL 1 DAY";
            break;
        case 'thisWeek':
            $whereConditions[] = "YEARWEEK(l.created_at, 1) = YEARWEEK(CURDATE(), 1)";
            break;
        case 'thisMonth':
            $whereConditions[] = "MONTH(l.created_at) = MONTH(CURDATE()) AND YEAR(l.created_at) = YEAR(CURDATE())";
            break;
        case 'thisYear':
            $whereConditions[] = "YEAR(l.created_at) = YEAR(CURDATE())";
            break;
        case 'dateRange':
        ...
        <div class="modal-content">
            ...
            <form id="filterForm" class="row" method="get">
                ...
                <div class="collapse row" id="moreFilters">
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="livestock" class="form-label">Livestock <strong>(Contains)</strong></label>
                        <input type="text" id="livestock" name="livestock" class="form-control" placeholder="Enter livestock">
                    </div>

                    <div class="col-md-3 mb-3">
                        <label for="numHeadsComp" class="form-label">Number of Heads</label>
                        <div class="input-group">
                            <select id="numHeadsComp" name="numHeadsComp" class="form-select">
                                <option value="exact">Is Exact</option>
                                <option value="lessThan">Less Than</option>
                                <option value="greaterThan">Greater Than</option>
                            </select>
                            <input type="number" id="numHeads" name="numHeads" class="form-control" placeholder="Type here...">
                        </div>
                    </div>
